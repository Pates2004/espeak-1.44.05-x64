...
$x64ProjectDir = Join-Path $projectRoot 'platforms\windows\x64'
$releaseDir = Join-Path $projectRoot 'build\x64\Release'
$stageDir = Join-Path $projectRoot 'build\package'
$installerDir = Join-Path $projectRoot 'build\installer'
$varioPublishDir = Join-Path $projectRoot 'build\vario\win-x64'
...
foreach ($project in $projects) {
    & $msbuild (Join-Path $x64ProjectDir $project) /m /nologo /v:minimal /p:Configuration=Release /p:Platform=x64
...
& dotnet publish (Join-Path $projectRoot 'platforms\windows\Vario\Vario.csproj') `
    -c Release -r win-x64 --self-contained true -o $varioPublishDir
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath (Join-Path $varioPublishDir 'Vario.exe'))) {
    throw 'The Vario x64 build failed.'
}
...
if (-not $SkipInstaller) {
    ...
    & $iscc (Join-Path $projectRoot 'platforms\windows\make_espeak.iss')
    if ($LASTEXITCODE -ne 0) {
...
if (-not $SkipInstaller) {
    Write-Host "Installer: $(Join-Path $installerDir 'setup_espeak-1.44.05-x64-r23.exe')"
}
