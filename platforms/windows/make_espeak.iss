; Inno Setup 6 installer for the native 64-bit Windows build of eSpeak 1.44.05

[Setup]
AppName=eSpeak
AppId={{6C3C2E57-862C-49B3-91F5-7A27FE8CC0D6}
AppVersion=1.44.05
AppVerName=eSpeak version 1.44.05 r26 (64-bit)
...
OutputBaseFilename=setup_espeak-1.44.05-x64-r26
...
VersionInfoDescription=eSpeak 1.44.05 r26 native 64-bit installer
...
begin
  Result := 0;
  len := Length(Lcode);
  if RegGetSubkeyNames(HKEY_LOCAL_MACHINE_64, RegPhConv, Convertors) then
  begin
    for ix := 0 to GetArrayLength(Convertors) - 1 do
    begin
      s1 := RegPhConv + '\' + Convertors[ix] + '\Attributes';
      if RegQueryStringValue(HKEY_LOCAL_MACHINE_64, s1, 'Language', sLangs) then
      begin
...
begin
  ...
  begin
    ...
    if RegQueryStringValue(HKEY_LOCAL_MACHINE_64, s1, 'Language', sLangs) then
    begin
      done := RegWriteStringValue(HKEY_LOCAL_MACHINE_64, s1, 'Language', Lcode + ';' + sLangs);
    end;

    if done = False then
    begin
      s1 := RegPhConv + '\eSpeak\Attributes';
      if RegQueryStringValue(HKEY_LOCAL_MACHINE_64, s1, 'Language', sLangs) = False then
      begin
        // add 'eSpeak' dummy PhoneConvertor
        RegWriteStringValue(HKEY_LOCAL_MACHINE_64, RegPhConv + '\eSpeak', 'CLSID','{9185F743-1143-4C28-86B5-BFF14F20E5C8}');
        RegWriteStringValue(HKEY_LOCAL_MACHINE_64, RegPhConv + '\eSpeak', 'PhoneMap','- 0001');
        RegWriteStringValue(HKEY_LOCAL_MACHINE_64, s1, 'Language','');
      end;
...
begin
  ...
  RegWriteStringValue(HKEY_LOCAL_MACHINE_64,RegVoice2,'',sEspeak+VoiceUC);
  ...
  RegWriteStringValue(HKEY_LOCAL_MACHINE_64,RegVoice2a,'Age','Adult');
  RegWriteStringValue(HKEY_LOCAL_MACHINE_64,RegVoice2a,'Language',Lcode);
...
// remove all espeak voices from the registry
  ...
  while exists do begin
    ...
    exists := RegKeyExists(HKEY_LOCAL_MACHINE_64,RegVoice2);
    RegDeleteKeyIncludingSubkeys(HKEY_LOCAL_MACHINE_64,RegVoice2);
    Index := Index + 1;
  end;
end;
