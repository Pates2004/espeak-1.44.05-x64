# eSpeak 1.44.05 — Windows x64

This directory contains native MSVC x64 projects for:

- `espeak.exe` — command-line synthesizer with a WinMM audio backend;
- `espeak_lib.dll` — public eSpeak API library;
- `espeak_sapi.dll` — 64-bit SAPI 5 engine;
...
```powershell
powershell -ExecutionPolicy Bypass -File platforms\windows\build-x64.ps1
```
...
same script to publish the self-contained x64 Vario voice manager. The regular
installer places Vario beside eSpeak and adds it to the eSpeak Start menu
group. Vario manages this edition's 64-bit SAPI registry view and its optional
Sonic speed boost.
